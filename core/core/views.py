...
class PaymentView(generic.View):
    def get(self, *args, **kwargs):
        ...
        if order.billing_address:
            ...
            messages.warning(self.request, "You have added  a billing address")
            return redirect("core:checkout")
    ...
    def post(self, *args, **kwargs):
        ...
        try:
            ...
            messages.success(self.request, "your order successful")
            return redirect("/")
            ...
            messages.error(self.request, "Authentication Stripe Api error")
            return redirect("/")

        except stripe.error.APIConnectionError as e:
            # Network communication with Stripe failed
            messages.error(self.request, "Network failed Error")
            return redirect("/")

        except stripe.error.StripeError as e:
            # Display a very generic error to the user, and maybe send
            messages.error(self.request, "You were not charged")
            return redirect("/")
            # yourself an email
...
def add_to_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            order_item.save()
            messages.info(request, "This item cart was updated.")
            return redirect("core:order-summary")
        else:
...
def remove_from_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            messages.info(request, "This item was remover from your cart")
            return redirect("core:order-summary")
        else:
            messages.info(request, "This item was in your cart")
